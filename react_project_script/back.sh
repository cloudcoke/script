...
INSTANCE=$(aws ec2 run-instances --image-id $UBUNTU --count $NUM --instance-type t2.micro --key-name $KEYPAIR_NAME --security-group-ids $SG --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=backend}]' --output text --query 'Instances[0].InstanceId')
if [ $? -ne 0 ]; then
