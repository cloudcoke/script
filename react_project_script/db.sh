...
## 포트를 배열로 선언
declare -a PORTS=("22" "80" "3306")

for PORT in "${PORTS[@]}"
do
    if [ $PORT -eq "3306" ]; then
        BACK_SERVER=$(nslookup $BACK_DNS | awk '/^Address: / { print $2 }')
        aws ec2 authorize-security-group-ingress --group-id $SG --protocol tcp --port $PORT --cidr $BACK_SERVER/32 > /dev/null 2>&1
        continue
    fi
...
INSTANCE=$(aws ec2 run-instances --image-id $UBUNTU --count $NUM --instance-type t2.micro --key-name $KEYPAIR_NAME --security-group-ids $SG --tag-specifications 'ResourceType=instance,Tags=[{Key=Name,Value=database}]' --output text --query 'Instances[0].InstanceId')
if [ $? -ne 0 ]; then
...
while :; do
    ...
    if [ $STATUS_CODE -eq 16 ]; then
        ...
        aws ec2 create-tags --resources $ALLOCATION_ID --tags Key=Name,Value=database
         if [ $? -ne 0 ]; then
